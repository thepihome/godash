...
__export(matches_exports, {
  autoMatchByClassification: () => autoMatchByClassification,
  handleMatches: () => handleMatches
});
async function autoMatchByClassification(env, jobId) {
  try {
    ...
    for (const candidate of candidates) {
      ...
      if (resume.skills && job.required_skills) {
        try {
          const resumeSkills = typeof resume.skills === "string" ? JSON.parse(resume.skills) : resume.skills;
          ...
          if (Array.isArray(resumeSkills) && Array.isArray(jobSkills)) {
            const matchingSkills = resumeSkills.filter(
              (skill) => jobSkills.some((js) => js.toLowerCase() === skill.toLowerCase())
            );
      ...
      if (existingMatch) {
        ...
        await execute(
          ...
          [
            ...
            matchScore,
            0,
            // skills_match - calculated separately if needed
...
var init_matches = __esm({
  "src/routes/matches.js"() {
    ...
    init_cors();
    init_auth();
    __name(autoMatchByClassification, "autoMatchByClassification");
  ...
  if (path === "/api/jobs" && method === "POST") {
    ...
    try {
      ...
      if (job_classification) {
        try {
          const { autoMatchByClassification: autoMatchByClassification2 } = await Promise.resolve().then(() => (init_matches(), matches_exports));
          await autoMatchByClassification2(env, jobId);
        } catch (e) {
...
__name(buildFilterConditions, "buildFilterConditions");
